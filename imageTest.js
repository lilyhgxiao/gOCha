...
app.get("/images/:id", uploads.single('avatar'), (req, res) => {
    const id = req.params.id;
    ...
    Image.findById(id)
        .then(image => {
            if (!image) {
                ...
                /// sometimes we wrap returned object in another object:
                //res.send({student})
                fs.writeFileSync(__dirname + '\\' + req.body.imagePath, image.img.data)
                result = {"id": image._id}
...
app.post("/images", uploads.single('avatar'), (req, res) => {
    const newItem = new Image();
    newItem.img.data = fs.readFileSync(__dirname + '\\' + req.body.imagePath)
    newItem.img.contentType = 'image/png';
    newItem.save().then(image => {
        const result = {"id": image._id}
        res.status(200).send(result);
    });  
})

app.post('/uploadfile', uploads.single('myFile'), (req, res, next) => {
    const file = req.file
    if (!file) {
